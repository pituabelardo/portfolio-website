...
(function () {
  ...
  function placeFlotsam() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      var left = i % 2 === 0;
      /* v8: right-side dwellers anchor with `right` so they can never
         overhang the viewport — an svg poking past 100vw made mobile
         browsers widen the layout viewport and render the page zoomed out */
      var xPct = 1.5 + seededRand(i, left ? 1 : 2) * 6;
      var yAbs = docH * (0.10 + (i + seededRand(i, 3)) / (n + 1) * 0.82);
      el.style[left ? "left" : "right"] = xPct + "vw";
